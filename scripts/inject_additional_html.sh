...
docker-compose exec postgres psql -U moodle -d moodle -c "insert into mdl_config('name', 'value') values('additionalhtmlhead', '$additional_html') on conflict('additionalhtmlhead') do update set value