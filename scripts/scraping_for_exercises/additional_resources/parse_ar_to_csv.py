...
INPUT_DIR = '../../../grouped_html/additionalresources/thinslice'
OUTPUT_CSV = 'additional_resources_no_TEKS.csv'
...
def split_by_try_it(soup):
    try_it_headers = []
    for tag in soup.find_all(['h1', 'h2', 'h3', 'h4', 'h5', 'h6']):
        if tag.get_text(strip=True).lower().replace('-', ' ').find('try it') >= 0:
            try_it_headers.append(tag)

    if len(try_it_headers) > 1:
        return 'MULTIPLE_TRY_IT', None, None
    elif len(try_it_headers) == 0:
        return 'NO_TRY_IT', soup, None

    split_header = try_it_headers[0]
    ...
    for elem in soup.contents:
        ...
        if not found_first_interactive:
            if is_interactive:
                ...
                # Remove header tags from the beginning of background
                trimmed_background_parts = [
                    e for e in background_parts
                    if not (e.name in ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'])
                ]
                background = ''.join(str(e) for e in trimmed_background_parts).strip()
...
def main():
    ...
    for file in sorted(Path(INPUT_DIR).glob('*.html')):
        ...
        filename = file.name
        status, before_soup, after_soup = split_by_try_it(soup)

        if status == 'MULTIPLE_TRY_IT':
            log_issue(logs, f"Multiple 'Try It' headers found in file {filename}. Skipping.")
            continue

        if status == 'NO_TRY_IT':
            log_issue(logs, f"No 'Try It' header in file {filename}. Treating entire file as one section.")
            if has_interactive_blocks(before_soup):
                multi_step_id = f"AR_{multi_step_counter}_AR"
        ...
        # status == 'SPLIT'
        if has_interactive_blocks(before_soup):
            multi_step_id = f"AR_{multi_step_counter}"
            rows = process_section(before_soup, filename, multi_step_id, logs)
            all_rows.extend(rows)
            multi_step_counter += 1
        ...
        if has_interactive_blocks(after_soup):
            ...
            log_issue(logs, f"No interactive blocks found in Try-It section of {filename}. Skipping Try-It section.")
